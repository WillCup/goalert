...
func init() {
    ...
    flag.StringVar(&MMMmsgInfo.To, "to", "@all", "消息的接收人，可以在钉钉后台查看，可空。")
    flag.StringVar(&MMMmsgInfo.Agentid, "agentid", "", "AgentID，可以在钉钉后台查看，不可空。")
    flag.StringVar(&MMMmsgInfo.Corpid, "corpid", "", "CorpID，可以在钉钉后台查看，不可空。")
    flag.StringVar(&MMMmsgInfo.Corpsecret, "corpsecret", "", "CorpSecret，可以在钉钉后台查看，不可空。")
    flag.StringVar(&MMMmsgInfo.Msg, "msg", `{ "from": "WILL测试", "time": "2016.07.28 17:00:05", "level": "Warning", "name": "这是一个WILL测试（willtest.com）提供的HTTPAPI钉钉报警插件。", "key": "icmpping", "value": "30ms", "now": "56ms", "id": "xxxx7", "ip": "8.8.8.8", "color":"FF4A934A", "age":"3m", "recoveryTime":"2016.07.28 17:03:05", "status":"OK" }`, "Json格式的文本消息内容，不可空。")
    flag.StringVar(&MMMmsgInfo.Url, "url", "http://www.willtest.com/golang-zabbix-alter-to-dingding.html", "消息内容点击后跳转到的URL，可空。")
    flag.StringVar(&MMMmsgInfo.Style, "style", "json", "Msg的格式，可选json和xml，推荐使用xml（支持消息中含双引号），可空。")
...
%v`, msg))
    ...
    var dingMsg DingMsg
    //给dingMsg各元素赋值
    dingMsg.Touser = MMMmsgInfo.To
    dingMsg.Agentid = MMMmsgInfo.Agentid
    dingMsg.Msgtype = "oa"
    dingMsg.Oa.MessageURL = MMMmsgInfo.Url
    if alert.Url != "" {
    ...
    if alert.Status == "PROBLEM" {
        ...
        //  其他status状况处理
        dingMsg.Oa.MessageURL = "https://www.willtest.com/golang-zabbix-alter-to-dingding.html"
        dingMsg.Oa.Body.Content = "HTTPAPI动作配置有误，请至WILL测试[willtest.com]或直接[点击此消息]查看具体配置文档。"
...
// func main() {
//  SendMsg(getToken(MMMmsgInfo.Corpid, MMMmsgInfo.Corpsecret), MakeMsg(MMMmsgInfo.Msg))
// }
